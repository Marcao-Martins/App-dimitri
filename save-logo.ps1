# Script para salvar a logo GDAV no projeto
# Execute este script depois de copiar a imagem GDAV

param(
    [string]$ImagePath
)

$targetPath = "assets\images\gdav_logo.png"

Write-Host "`n╔══════════════════════════════════════════════╗" -ForegroundColor Cyan
Write-Host "║      📸 Salvando Logo GDAV no Projeto      ║" -ForegroundColor Cyan
Write-Host "╚══════════════════════════════════════════════╝`n" -ForegroundColor Cyan

# Verifica se o caminho da imagem foi fornecido
if (-not $ImagePath) {
    Write-Host "❌ Por favor, forneça o caminho da imagem GDAV" -ForegroundColor Red
    Write-Host "`nUso:" -ForegroundColor Yellow
    Write-Host "  .\save-logo.ps1 -ImagePath 'C:\caminho\para\gdav_logo.png'`n" -ForegroundColor White
...
try {
    ...
    Write-Host "✓ Logo salva com sucesso em: $targetPath" -ForegroundColor Green
    ...
    Write-Host "   • Página de perfil`n" -ForegroundColor Gray
